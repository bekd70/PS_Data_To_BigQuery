/**
 * goes through each table on the dailyImport sheet
 * for each table:
 ...
 */
function getDailyPsData() {
  let log = [];
  let logType = "[LOG]";
  let runTime = 0;
  let tableNames = ss.getSheetByName(GLOBAL.psDailyImportTablesSheet).getDataRange().getValues();
  for(let i=1;i<tableNames.length;i++){
    ...
    /**
     * TODO: get Data from Powerschool
    ...
    if(rowsToProcess>0){
      ...
      do{
        //gets query to run from the last row of each tableSheet
